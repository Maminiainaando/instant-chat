...
const listUsers = (req, res) => {
  pool.query('SELECT username,status_user,image_user FROM users ORDER BY id_user ASC', (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: 'An error occurred while fetching users' });
    }
...
const getAllMessage = (request, response) => {
  ...
  pool.query('SELECT messages, sending_date FROM messages WHERE id_user = $1', [id], (error, results) => {
    if (error) {
      throw error;
    }
...
const createUser = (request, response) => {
  ...
  pool.query('INSERT INTO users (username, email, password, image_user) VALUES ($1, $2, $3, $4) RETURNING id_user',
    [username, email, password, image_user], (error, results) => {
    if (error) {
      throw error;
    }
...
const createMessage = (request, response) => {
  ...
  pool.query('INSERT INTO messages (messages, id_user, sending_date, date_created, type_message) VALUES ($1, $2, $3, $4, $5) RETURNING id_message',
    [messages, id_user, sending_date, date_created, type_message], (error, results) => {
    if (error) {
      throw error;
    }
...
const updatePassword = (request, response) => {
  ...
  pool.query('UPDATE users SET password = $1 WHERE id_user = $2', [password, id], (error, results) => {
    if (error) {
      throw error;
    }
...
const updateStatus = (request, response) => {
  ...
  pool.query('UPDATE users SET status_user = $1 WHERE id_user = $2', [status_user, id], (error, results) => {
    if (error) {
      throw error;
    }
    response.status(200).send(`User modified with ID: ${id}`);
